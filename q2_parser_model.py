...
class ParserModel(Model):
    ...
    def create_feed_dict(self, inputs_batch, dropout=1, labels_batch=None):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        allEmbs = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(allEmbs, self.input_placeholder)
    ...
    def add_prediction_op(self):
       ...
       # V = tf.Variable(xavier_initializer((self.config.hidden2_size, self.config.hidden_size)))
        U = tf.Variable(xavier_initializer((self.config.hidden_size, self.config.n_classes)))
       # b1 = tf.Variable(tf.zeros((self.config.hidden2_size, )))
        b2 = tf.Variable(tf.zeros((self.config.hidden_size, )))
        b3 = tf.Variable(tf.zeros((self.config.n_classes, )))
        #h1 = tf.nn.relu(tf.matmul(x,W) + b1 )
        #h1_drop = tf.nn.dropout(h1, self.dropout_placeholder)
        h = tf.nn.relu(tf.matmul(x,W) + b2 )
